...
def build_acoustic(spec: Dict[str, Any], Sd: float | None):
	...
	if t == "horn":
		# Horn: route by profile; generic uses area_list only; conical/exponential use L,S_throat,S_mouth
		...
		if profile in ("con", "conical", "cone", "exp", "exponential"):
			for kreq in ("L", "S_throat", "S_mouth"):
				if kreq not in spec:
					raise KeyError(f"horn: missing required key '{kreq}' for profile={profile}")
			if "area_list" in spec and spec["area_list"] is not None:
				raise ValueError(f"horn(profile={profile}): area_list is not allowed.")
			return Horn(L=float(spec["L"]), S_throat=float(spec["S_throat"]), S_mouth=float(spec["S_mouth"]), profile=profile, R_throat=R_throat, R_mouth=R_mouth, mouth_load=mouth_load, mouth_label=mouth_label, throat_load=throat_load, throat_label=throat_label, mouth_termination=mouth_termination)
		elif profile == "generic":
			if "area_list" not in spec:
				raise KeyError("horn(profile=generic): missing required key 'area_list'")
			return Horn(profile="generic", area_list=list(spec["area_list"]), R_throat=R_throat, R_mouth=R_mouth, mouth_load=mouth_load, mouth_label=mouth_label, throat_load=throat_load, throat_label=throat_label, mouth_termination=mouth_termination)
		else:
