...
def build_registry(cfg: dict):
    ...
    # First pass: everything but drivers
    for e in elems:
        typ = (e.get("type") or "").lower()
        lab = e["label"]
        if typ in ("re","resistor"):
            reg[lab] = Re(R=float(e["R"]))
        elif typ in ("le","inductor"):
            reg[lab] = Le(L=float(e["L"]))
            ...
            Sd_hint = float(e.get("Sd_m2", 0.053))  # optional hint
    ...
    # Second pass: drivers (need front/back labels)
    for e in pending_drivers:
        lab = e["label"]
        Sd = float(e.get("Sd_m2", 0.053))
        ...
        mot = DriverMechanicalBranch(
            Rms_val=float(e.get("Rms", 1.6)),
            Mms_val=float(e.get("Mms", 0.020)),
            Cms_val=float(e.get("Cms", 7.0e-4)),
            front_load=reg[front_lab],
            back_load=reg[back_lab],
            Sd=Sd,
        )
        drv = Driver(
            Re_val=float(e.get("Re", 5.7)),
            Le_val=float(e.get("Le", 0.35e-3)),
            Bl=float(e.get("Bl", 7.4)),
            motional=mot,
        )
        reg[lab] = drv

    # Expect exactly one driver (simple case)
    ...
    driver_label = drivers[0]
    return reg, driver_label

# ---------------- Network resolver ----------------
def build_net(node, reg):
    """node: either a label (str) or {'series':[...]} or {'parallel':[...]} or list (implicit series)."""
...
def build_system(cfg: dict):
    ...
    angles = np.array(cfg.get("angles_deg", []), dtype=float) if cfg.get("angles_deg") else None

    # Extract loading label from driver's front radiation if present
    loading_label = "4pi"
    from .driver import Driver as DriverClass
    for k,v in reg.items():
        if isinstance(v, DriverClass):
            front_load = v.motional.front_load
            loading_label = getattr(front_load, "loading", "4pi")
            break

    driver = reg[drv_label]
    return f, w, net, driver, driver.motional.Sd, Vsrc, r, loading_label, angles

def main(argv=None):
    parser = argparse.ArgumentParser(description="SpeAcouPy: labeled elements + label-referenced networks")
