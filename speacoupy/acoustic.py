...
class Horn(Acoustic):
	...
	def _abcd_chain(self, omega, N=512):
		"""
		Impedance marching using Riccati equation for spherical/conical waves.
		"""
		import numpy as np
		from .constants import RHO0, C0

		nf = len(omega)

		# Discretize horn length
		x = np.linspace(0, self.L, N+1)
		delta_x = self.L / N

		# Area profile (linear interpolation throat→mouth for now)
		S = self.S_throat + (self.S_mouth - self.S_throat) * (x / self.L)

		# Derivative dS/dx (constant for conical, but code works for general flares too)
		dSdx = np.gradient(S, delta_x)

		Z_throat = np.zeros((nf,), dtype=complex)
		...
		for i, w in enumerate(omega):
			...
			if self.mouth_load == "rigid":
				...
				Z = RHO0 * C0 / self.S_mouth  # piston approx
			...
			for n in range(N, 0, -1):
				...
				def dZdx(Z):
					return 1j * RHO0 * w * (1 - (Z/(RHO0*C0))**2) - (dSdxn/Sn) * Z

				# RK4 integration step
				k1 = -dZdx(Z)
				k2 = -dZdx(Z + 0.5*delta_x*k1)
				k3 = -dZdx(Z + 0.5*delta_x*k2)
				k4 = -dZdx(Z + delta_x*k3)
				Z = Z + (delta_x/6)*(k1 + 2*k2 + 2*k3 + k4)
	...
	def _abcd_chain2(self, omega, N=128):
		"""
		ABCD chain using **piecewise conical frusta** (spherical-wave exact per slice)
		with cylindrical fallback if the local radius change is negligible.
		Uses global constants RHO0, C0 and the horn's own area profile self._area(x).
		"""
		import numpy as np
		...
		# Slice boundaries and areas from the actual horn profile
		x_nodes = np.linspace(0.0, self.L, N + 1)
		S_nodes = self._area(x_nodes)
		S_nodes = np.maximum(S_nodes, 1e-30)

		# Linear stuffing profile at slice centers (Pa·s/m^3)
		if self.L > 0.0:
		...
		# Initialize total ABCD (identity)
		...
		# Helper: exact spherical two-port for one conical frustum
		def frustum_abcd(S1: float, S2: float, R1: float, R2: float):
			Zstar = (RHO0 * C0) / np.sqrt(S1 * S2)
			delta = k * (R2 - R1)                # = k * ell
			kr1 = k * R1
			kr2 = k * R2
			# guards for near-cylindrical limits
			...
			B = -1j * Zstar * sind
			C = -1j / Zstar * (
				sind * (1.0 + 1.0 / (kr1 * kr2)) + (R1 - R2) * cosd / (kr1 * kr2)
			)
			D = (kr2 * cosd - sind) / kr1
			return A, B, C, D
		...
		for i in range(N):
			...
			ell = x_nodes[i + 1] - x_nodes[i]

			# Local radii and slope
			a1 = (S1 / np.pi) ** 0.5
			a2 = (S2 / np.pi) ** 0.5
			alpha = (a2 - a1) / max(ell, 1e-30)

			if abs(a2 - a1) <= rel_tol * max(a1, 1e-12):
				# Cylindrical slice
				S_mid = (S1 * S2) ** 0.5
				Zc = (RHO0 * C0) / S_mid
				delta = k * ell
				...
				# Conical frustum: distances to the (local) cone apex
				R1 = a1 / alpha
				R2 = R1 + ell
				A, B, C, D = frustum_abcd(S1, S2, R1, R2)
	...
	def _abcd_chainPLAINWAVESMAYBE(self, omega: np.ndarray, N: int = 64):
		"""
